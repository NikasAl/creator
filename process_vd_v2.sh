...
# process_vd_v2.sh
# РЕФАКТОРИНГ: Использует lib/common/ вместо кросс-импортов
...
if [ $# -eq 0 ]; then
    ...
    echo "./process_vd_v2.sh $CONFIG_FILE"
    exit 0
else
    CONFIG_FILE="$1"
...
# VD-специфичные модули (обновлённые)
source "lib/vd/utils_v2.sh"
...
log_header "🎬 Обработка видео: $TITLE"
...
common_step_add_music "$OUTPUT_DIR" "$OUTPUT_DIR/video.mp4"
